...
LoadDualPCM:
		...
		move.w	#$0100,$A11100%laddrt%	; request Z80 stop
		move.w	#$0100,$A11200%laddrt%	; Z80 reset off

		lea	DualPCM,a0		; load Dual PCM address into a0
		lea	dZ80%laddr%,a1%kt%		; load Z80 RAM address into a1

.z80
		btst	#$00,$A11100%laddr%		; check if Z80 has stopped
...
.ntsc
		move.b	d0,dZ80+YM_FlushTimer+2%laddr%%kt%; save flush timer

		move.w	#$0000,$A11200%laddrt%	; request Z80 reset
		moveq	#$7F,d1			; wait for a little bit
		dbf	d1,*			; we can't check for reset, so we need to delay

		move.w	#$0000,$A11100%laddrt%	; enable Z80
		move.w	#$0100,$A11200%laddrt%	; Z80 reset off
