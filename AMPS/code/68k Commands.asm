...
dcskip	macro amount
	if %macpfx%amount=0
		...
		addq.w	#%macpfx%amount,a2		; skip this amount of bytes
	endif
	rts
   endm
...
; ---------------------------------------------------------------------------
		...
		lea	dZ80+PCM_Volume1%laddr%,a4	; get Z80 volume table to a4
...
.volloop
	...
	%endr%					; this actually saves some cycles
		...
		move.b	d3,dZ80+PCM1_VolumeCur+1%laddr%; set PCM1 volume as mute
		move.b	d3,dZ80+PCM2_VolumeCur+1%laddr%; set PCM2 volume as mute
	startZ80
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
		...
		move.b	#$FF,dPSG%laddr%		; mute PSG4
...
dUpdateVoiceFM:
	...
	dCALC_VOICE				; get address of the specific voice to a4
		...
		moveq	#%mq%B0,d3%at%		; YM command: Algorithm & FeedBack
		or.b	d2,d3			; add channel offset to register
		move.b	d3,(a5)+		; write register to buffer
...
; ---------------------------------------------------------------------------
		...
		moveq	#%mq%B4,d3%at%		; YM command: Panning & LFO
		or.b	d2,d3			; add channel offset to register
		move.b	d3,(a5)+		; write register to buffer
