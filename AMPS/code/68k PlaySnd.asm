...
; ---------------------------------------------------------------------------
		...
		moveq	#%mq%B4,d5%at%		; YM address: Panning and LFO
...
dMuteDAC:
	stopZ80					; wait for Z80 to stop
		...
		lea	dZ80+PCM1_Sample%laddr%,a4	; load addresses for PCM 1 sample to a4
	...
	%endr%
		...
		lea	SampleList(pc),a5	; load address for the stop sample data into a5
		lea	dZ80+PCM2_Sample%laddr%,a4	; load addresses for PCM 2 sample to a4

	rept 12
		move.b	(a5)+,(a4)+		; send sample data to Dual PCM
	%endr%

		move.b	#$CA,dZ80+PCM1_NewRET%laddr%	; activate sample switch (change instruction)
		move.b	#$CA,dZ80+PCM2_NewRET%laddr%	; activate sample switch (change instruction)
...
%pcreldef% .index MusicIndex-(MusOff*4)		; effin ASS
		lea	%pcrelref%(pc),a2%at%%at%; get music pointer table with an offset
...
dMutePSG:
		lea	dPSG%laddr%,a4%kt%		; load PSG data port address to a4
